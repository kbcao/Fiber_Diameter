...
__temp_all_x = np.reshape(vec1, [-1, cnn_size, cnn_size, 3])
__temp_all_y = np.reshape(vec2, [-1, cnn_size * cnn_size])
...
all_data_x = np.reshape(vec1, [-1, cnn_size, cnn_size, 3])
all_data_y = np.reshape(vec2, [-1, cnn_size * cnn_size])
...
if is_training:
    print('Training ------------')
    model.fit(training_data_x, training_data_y, epochs=train_apochs, batch_size=1, verbose=1,
    ...
    print('\nTesting ------------')
    loss, accuracy = model.evaluate(test_data_x, test_data_y, batch_size=1,)
    prediction_data = model.predict(test_data_x, batch_size=1)
    ...
    plt.figure()
    plt.plot(list(range(len(test_data_y_unscale) * cnn_size * cnn_size)), test_data_y_unscale.flatten(), color='b',
             label='Original data')
    plt.plot(list(range(len(prediction_data) * cnn_size * cnn_size)), prediction_data.flatten(), color='r',
